...
(define (m*vector-array! matrix vector #!key (stride 0) (length 0))
  (cond
   ...
   ((u8vector? vector)
    ...
    ((cond
      ((f32vector? matrix)
       (foreign-lambda void "hpmMat4VecArrayMult" f32vector (c-pointer float) size_t size_t))
      ((or (pointer? matrix) (locative? matrix))
       (foreign-lambda void "hpmMat4VecArrayMult" c-pointer (c-pointer float) size_t size_t))
      (else (error 'm*vector-array! "Wrong argument type" matrix)))
