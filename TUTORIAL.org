...
Let us see if we can top that by loading a texture using the function ~(texture:load filename)~. The most simple way of displaying a texture is making a rendering function from it using ~(texture:renderer texture [rect])~ calling the function returned by this will render the texture to screen it to screen. Let's try:
: (define (game)
:   (let* ((texture  (texture:load "./share/grid.png"))
:          (renderer (texture:renderer grid-texture)))
:    (lambda (#!optional (i 0))
:      (grid-renderer)
:      (if (= i 100) #f
:          (+ i 1)))))
